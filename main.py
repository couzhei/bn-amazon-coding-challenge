...
def number_one():
    while True:
        ...
        return generate_graph(n_nodes=inpt)
    

def number_two():
    print("Please enter starting point (x, y):", end="")

    f = lambda x:tuple(map(int, tuple(x.rstrip(")").lstrip("(").split(","))))

    start = f(input())

    print("Now enter delivery point:", end="")
...
def number_three(obstacles, start, destination):

    print("Enter the obstacles separated by space using the format (x1,y1) (x2,y2), etc., in case you want to add a few random obstacles to your previous ones, enter r:", end="")

    inpt = input()

    if inpt == "r":
        print("How many random obstacles:")

        n_obstacles = int(input())
        ...
        return set(map(f, inpt)) - {start, destination}

def number_four(graph, obstacles, start, destination):
    if graph and start:
        n_steps, steps, distance = shortest_path(graph, obstacles, start, destination)
        if steps:
            print("Steps are as follows:", steps)
        else:
            print("Unable to reach delivery point.")
            to_be_removed = remove_obstacles(graph, obstacles, start, destination)
            print(to_be_removed[0])
    else:
        print("Either graph or starting and destination point hasn't been set")
...
if __name__ == '__main__':
    ...
    while inpt != 5:
        print("""
    +---------------------------------------------------------+
    |1. Generate a new graph (default is 10x10).              |
    |2. Enter starting and destination points.                |
    |3. Introduce some obstacle(s).                           |
    |4. Show the shortest path.                               |
    |5. Exit.                                                 |
    +---------------------------------------------------------+
        """)

        if start:
            print(f"starting = {start} and delivery point = {destination}\n")

        if obstacles:
            print(f"Obstacles:{obstacles}\n")

        print("Enter the task number:", end="")
        ...
        try:
            ...
            print("\nWrong input! Moving back to menu.", end="")
            for i in "...":
        ...
        if inpt == 1:
            ...
            start, destination = number_two()
            continue

        elif inpt == 3:
            if not start:
                print("First enter starting point and delivery point")
                continue
            obstacles = number_three(obstacles, start, destination)

        elif inpt == 4:
            x = number_four(my_graph, obstacles, start, destination)
            continue
